...
@section('content')
    <div class="container mx-auto mt-6">
        ...
        <div class="bg-sky-100 p-6 rounded shadow-md mb-6 text-center">
            ...
            <p><strong>Nueva Hora de Devolución:</strong> {{ $nuevoHoraDevolucion }}</p>
        ...
        <div class="flex gap-4 justify-center">
            <form method="POST" action="{{ route('reservar.confirmarModificacion') }}">
                ...
                <button type="submit"
                    class="bg-lime-500 hover:bg-lime-700 text-white font-bold py-2 px-4 rounded">Modificar Reserva</button>
            </form>
            <form method="POST" action="{{ route('reservas.rechazarModificacion') }}">
                @csrf
                <input type="hidden" name="id_reserva" value="{{ $reserva->id_reserva }}">
                <button type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Rechazar
                    Reserva</button>
            </form>
        </div>
    </div>
