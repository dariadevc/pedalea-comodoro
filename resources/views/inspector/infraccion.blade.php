...
@section('contenido')
    ...
    <form action="{{ route('infraccion.generar') }}" method="POST"
        ...
        <div class="mb-4 w-full">
            <!-- Campo de patente -->
            <label for="patente" class="mt-4 text-left text-slate-50 border-b-2 border-slate-50 mb-2">
                Ingrese la patente de la bicicleta
            </label>
            <input type="text" name="patente" id="patente"
                class="w-full mt-3 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 bg-gray-50 border-b-2 text-pc-texto-p mb-4"
                placeholder="Ej: A05" required>
            @error('patente')
            <span class="text-white text-sn">{{ $message }}</span>
            @enderror
            ...
            <p class="text-white">Tiene que ser entre 10 y 200</p>
            <input type="number" name="puntos" id="puntos" min="1" step="1"
                class="w-full mt-3 mb-4 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 bg-gray-50 border-b-2 text-pc-texto-p"
            ...
            <textarea name="motivo" id="motivo" rows="5"
                class="w-full mt-3 mb-4 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 bg-gray-50 border-b-2 text-pc-texto-p resize-none"
